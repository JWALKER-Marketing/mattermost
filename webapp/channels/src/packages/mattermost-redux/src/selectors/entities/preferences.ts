...
export function getFromPreferences(
    preferences: PreferencesType,
    category: string,
    name: string,
    defaultValue: any = ""
...
export function makeGetCategory(
    selectorName: string,
    category: string
): (state: GlobalState) => PreferenceType[] {
    return createIdsSelector(selectorName, getMyPreferences, (preferences) => {
...
export function makeGetUserCategory(
    selectorName: string,
    category: string
): (state: GlobalState, userID: string) => PreferenceType[] {
    return createIdsSelector(
        selectorName,
        (state: GlobalState, userID: string) =>
            getUserPreferences(state, userID),
        (preferences) => {
            const prefix = category + "--";
            const prefsInCategory: PreferenceType[] = [];
...
export const getDirectShowPreferences = makeGetCategory(
    "getDirectShowPreferences",
    Preferences.CATEGORY_DIRECT_CHANNEL_SHOW
);
export const getGroupShowPreferences = makeGetCategory(
    "getGroupShowPreferences",
    Preferences.CATEGORY_GROUP_CHANNEL_SHOW
);

export const getTeammateNameDisplaySetting: (state: GlobalState) => string =
    createSelector(
        ...
        (config, teammateNameDisplayPreference, license) => {
            const useAdminTeammateNameDisplaySetting =
                license &&
                license.LockTeammateNameDisplay === "true" &&
...
export const getThemePreferences = makeGetCategory(
    "getThemePreferences",
...
const getDefaultTheme = createSelector(
    ...
    (config): Theme => {
        if (config.DefaultTheme && config.DefaultTheme in Preferences.THEMES) {
            const theme: Theme =
                Preferences.THEMES[config.DefaultTheme as ThemeKey];
            if (theme) {
                return theme;
            }
        }
...
export function makeGetStyleFromTheme<Style>(): (
    state: GlobalState,
    getStyleFromTheme: (theme: Theme) => Style
) => Style {
    return createSelector(
        "makeGetStyleFromTheme",
        getTheme,
        (state: GlobalState, getStyleFromTheme: (theme: Theme) => Style) =>
            getStyleFromTheme,
...
export function getUnreadScrollPositionPreference(
    state: GlobalState,
    userPreferences?: PreferencesType
): string {
    return get(
...
export function getCollapsedThreadsPreference(state: GlobalState): string {
    ...
    if (
        configValue === CollapsedThreads.DEFAULT_ON ||
        configValue === CollapsedThreads.ALWAYS_ON
    ) {
...
export function getCollapsedThreadsPreferenceFromPreferences(
    state: GlobalState,
    userPreferences: PreferencesType
): string {
    const configValue = getConfig(state)?.CollapsedThreads;
...
export function isCollapsedThreadsAllowed(state: GlobalState): boolean {
    return (
        Boolean(getConfig(state)) &&
        getConfig(state).CollapsedThreads !== undefined &&
...
export function isCollapsedThreadsEnabled(state: GlobalState): boolean {
    ...
    return (
        isAllowed &&
        (userPreference === Preferences.COLLAPSED_REPLY_THREADS_ON ||
            getConfig(state).CollapsedThreads === CollapsedThreads.ALWAYS_ON)
    );
}

export function isCollapsedThreadsEnabledForUser(
    state: GlobalState,
    userPreferences: PreferencesType
): boolean {
    const isAllowed = isCollapsedThreadsAllowed(state);
    const userPreference = getCollapsedThreadsPreferenceFromPreferences(
        state,
        userPreferences
    );
...
export function isGroupChannelManuallyVisible(
    state: GlobalState,
    channelId: string
): boolean {
    return getBool(
...
export function getHasDismissedSystemConsoleLimitReached(
    state: GlobalState
): boolean {
    return getBool(
        state,
...
export function syncedDraftsAreAllowedAndEnabled(state: GlobalState): boolean {
    const isConfiguredForFeature =
        getConfig(state).AllowSyncedDrafts === "true";
    const isConfiguredForUser = getBool(
        state,
        Preferences.CATEGORY_ADVANCED_SETTINGS,
...
export function getVisibleDmGmLimit(
    state: GlobalState,
    userPreferences?: PreferencesType
) {
...
export function moveThreadsEnabled(state: GlobalState): boolean {
    return (
        getFeatureFlagValue(state, "MoveThreadsEnabled") === "true" &&
        getLicense(state).IsLicensed === "true"
    );
}
...
export const getOverageBannerPreferences = makeGetCategory(
    "getOverageBannerPreferences",
    Preferences.CATEGORY_OVERAGE_USERS_BANNER
);
